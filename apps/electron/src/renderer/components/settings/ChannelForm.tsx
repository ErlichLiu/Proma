...
/** 所有可选供应商 */
const PROVIDER_OPTIONS: ProviderType[] = ['anthropic', 'openai', 'deepseek', 'google', 'moonshot', 'kimi-api', 'kimi-coding', 'zhipu', 'minimax', 'doubao', 'qwen', 'mimo', 'mimo-openai', 'custom']
...
function buildPreviewUrl(baseUrl: string, provider: ProviderType): string {
  ...
  if (provider === 'anthropic' || provider === 'deepseek' || provider === 'kimi-api' || provider === 'kimi-coding' || provider === 'mimo') {
...
export function ChannelForm({ channel, onSaved, onCancel }: ChannelFormProps): React.ReactElement {
  ...
  const handleProviderChange = (newProvider: string): void => {
    ...
    if (models.length === 0) {
      ...
      } else if (p === 'mimo' || p === 'mimo-openai') {
        setModels([
          { id: 'MiMo-7B-RL', name: 'MiMo 7B RL', enabled: true },
        ])
      }
    }
  }
