...
export class AnthropicAdapter implements ProviderAdapter {
  ...
  private normalizeUrl(baseUrl: string): string {
    // DeepSeek / Kimi：baseUrl 本身已含非版本路径（如 /anthropic、/coding/v1），
    // 不需要追加 /v1；由 channel-manager 的默认 URL 或用户自行保证路径完整
    // MiMo 的 Anthropic 端点需要 /v1（如 /anthropic/v1/messages），走标准规范化
    if (
